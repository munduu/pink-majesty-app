 ...
 */
 function register_event_handlers()
 {
	...
	function getLogin(login_email,login_senha,token_id){
		...
		if(login_email==''){
			alert('Campo obrigatÃµrio vazio: E-mail');
			return false;
		}
		if(login_senha==''){
			alert('Campo obrigatÃµrio vazio: Senha');
			return false;
		}
		$.ajax({
		type:"POST",
			...
			success: function(resultado){
				...
				if(resultado.erro == 2){
					if(resultado.tipo == 'Profissional'){
						$('.novo_pedido').show();
						//navigator.notification.alert(resultado.dados, '', 'ParabÃ©ns', 'OK'); 
					}
    ...
    $(document).on("click", ".btn_esqueci", function(evt)
    {
		...
		timeout: 100000, 
			...
			success: function(resultado){
				$('.loader').hide();
				if(resultado.erro==1){	
					alert('Confira seu e-mail com as instruÃ§Ãµes!');
				}else{
					alert(resultado.dados);
					activate_page("#login");
				}
			...
			error: function(resultado) {
				$('.loader').hide();
				alert('NÃ£o foi possÃ­vel acessar! #004');
				return false;
			}   
		});				
        return false;
	...
	*/
    ...
    {
		if(x == 0){
			$(".add_endereco").html('<div class="buttonentrar">Adicionar EndereÃ§o ja Existente</div>');
			$(".sel_endereco").hide();
			$(".cad_endereco").show();
			...
			$(".add_endereco").html('<div class="buttonentrar">Adicionar Novo EndereÃ§o</div>');
			$(".sel_endereco").show();
			$(".cad_endereco").hide();
			x=0;
		}
	...
	$(document).on("click", ".btn-cartao", function(evt)
    {
		...
		if(tipo==1){
			if(cpf_titular == ''){		
				alert('CPF do Titular NÃ£o Informado!');
				return false;
			}else if(numero_c == ''){		
				alert('Numero NÃ£o Informado!');
				return false;
			}else if(cod_seg == ''){			
				alert('Codigo de SeguranÃ§a NÃ£o Informado!');
				return false;
			}else if(mes == ''){				
				alert('Mes NÃ£o Informado!');
				return false;
			}else if(ano == ''){				
				alert('Ano NÃ£o Informado!');
				return false;
			}else if(nome_impresso == ''){	
				alert('Nome Impresso NÃ£o Informado!');
				return false;
			}else if(data_nasc_cartao == ''){
				alert('Data Nadcimento NÃ£o Informado!');
				return false;
			}else{
	...
	$(document).on("click", ".btn-data-hora", function(evt)
    {
		...
		if(info_data[2] == 'MarÃ§o'){	var mes = '03';}
	...
	$(document).on("click", ".cad_cliente", function(evt)
    {
		...
		for (var i = 0; i < sexo_cont.length; i++) {
        	if (sexo_cont[i].checked) {
			 ...
			 if(nome == ''){		alert('Nome Completo NÃ£o Informado!');}
		else if(tel2 == ''){		alert('Celular NÃ£o Informado!');}
		else if(sexo == ''){		alert('Sexo NÃ£o Informado!');}
		else if(data_n == ''){		alert('Data de Nascimento NÃ£o Informado!');}
		else if(email == ''){		alert('Email NÃ£o Informado!');}
