...
if($token=='H424715433852'){
	...
	if($linha > 0){
		...
		$updateSQL = "
		UPDATE tb_enderecos SET principal='0' WHERE id_cliente = '$id_cliente' AND principal = '1'";
		mysql_select_db($database_localhost, $localhost);
  		$Result1 = mysql_query($updateSQL, $localhost) or die(mysql_error()); 
    	salvaLog("Alterar tb_enderecos ID $id",$_SESSION['email']);
		
		$updateSQL = "
		UPDATE tb_enderecos SET principal='1' WHERE id = '$end'";
		mysql_select_db($database_localhost, $localhost);
  		$Result1 = mysql_query($updateSQL, $localhost) or die(mysql_error()); 
    	salvaLog("Alterar tb_enderecos ID $id",$_SESSION['email']);		
		
		$error = 2;
		$dados = "ENDEREÇO ALTERADO COM SUCESSO!";
	}else{
		$error = 3;
