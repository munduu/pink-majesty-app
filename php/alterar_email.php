...
$email 		= anti_sql_injection(strip_tags(trim($_POST['email'])));
...
if($token=='H424715433852'){
	...
	if($linha > 0){
		...
		$updateSQL = "
		UPDATE tb_cliente SET email='$email' WHERE id = '$id_cliente'";
		mysql_select_db($database_localhost, $localhost);
  		$Result1 = mysql_query($updateSQL, $localhost) or die(mysql_error()); 
    	salvaLog("Alterar tb_cliente ID $id",$_SESSION['email']);
		
		$updateSQL = "
		UPDATE tb_login SET email='$email' WHERE id_cliente = '$id_cliente'";
		mysql_select_db($database_localhost, $localhost);
  		$Result1 = mysql_query($updateSQL, $localhost) or die(mysql_error()); 
    	salvaLog("Alterar tb_login ID $id",$_SESSION['email']);		
		
		$error = 2;
		$dados = "EMAIL ALTERADO COM SUCESSO!";
	}else{
		$error = 3;
